...
GLOBAL_DEFINES += WITH_DEBUG_LOG_BUF=1 LK_LOG_BUF_SIZE=32768

# Keep the kernel command line clean when booting other operating systems
GLOBAL_DEFINES += GENERATE_CMDLINE_ONLY_FOR_ANDROID=1

# Disable reading splash partition to avoid crashes on some devices
GLOBAL_DEFINES += DISABLE_SPLASH_PARTITION=1

# Enable "fastboot oem help" for a list of supported fastboot commands
GLOBAL_DEFINES += FASTBOOT_HELP=1

# Allow entering fastboot after forced reset
GLOBAL_DEFINES := $(filter-out USER_FORCE_RESET_SUPPORT=1, $(GLOBAL_DEFINES))

# Allow flashing independent of battery voltage
GLOBAL_DEFINES := $(filter-out CHECK_BAT_VOLTAGE=1, $(GLOBAL_DEFINES))
