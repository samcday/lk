...
GLOBAL_DEFINES := $(filter-out LONG_PRESS_POWER_ON=1, $(GLOBAL_DEFINES))
...
GLOBAL_DEFINES := $(filter-out SECURE_CODE_MEM=1, $(GLOBAL_DEFINES))

# Weak battery charging is handled by the primary bootloader
GLOBAL_DEFINES := $(filter-out ENABLE_WBC=1, $(GLOBAL_DEFINES))

# Should be already done by primary bootloader if wanted
GLOBAL_DEFINES := $(filter-out ENABLE_XPU_VIOLATION=1, $(GLOBAL_DEFINES))
